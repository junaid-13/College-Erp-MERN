...
/**
 * Faculty submission review + grading (Task 13.30).
 */
export default function SubmissionReview() {
  const { id } = useParams();
  const [assessment, setAssessment] = useState(null);
  const [subs, setSubs] = useState([]);
  const [drafts, setDrafts] = useState({}); // { [submissionId]: { marks, feedback } }
  ...
  function setDraft(subId, key, value) {
  ...
  async function grade(sub) {
    ...
    const err = validateGrade(marks, assessment?.maxMarks);
    if (err) {
      setError(err);
      return;
    }
